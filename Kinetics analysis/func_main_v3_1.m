function func_main_v3_1(parameters)
...
for i=1:length(fileName)
    ...
    if smart_param == true
        ...
        if strcmp(ext,'.tif') || strcmp(ext,'.tiff')
            ...
            pro = prom_level;
            for j = 1:100
                prom_level(j) = 5*param.prominence_high/100*j;
                [~,~,wdt,pmn] = find_peaks_2D(I_int, prom_level(j));
                num_peaks(j) = length(wdt);
                wid(j) = mean(wdt);
                pro(j) = mean(pmn);
            end
            figure
    ...
    [tracks, track_props] = separate_tracks(spot_record);       %% It is calcualting all tracks even if too short. Change when you have time
