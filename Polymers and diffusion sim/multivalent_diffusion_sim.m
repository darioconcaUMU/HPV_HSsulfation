...
function [track, binders_loc] = multivalent_diffusion_sim(binders, kinetics, time)
...
% Dimension and time normalied to virion interaction area and 1/koff
...
% This introduces a correction term that takes into account the brownian
% motion of a tethered particle
r_int = (r_int*25 + 11.519*(1:200).^-0.3203)/25*0+r_int;
...
while ~isempty(particle_bonds) && time < max_time
    ...
    if ~isempty(free_binders)
        ...
        potential = free_binders(dists < r_int(min(length(particle_bonds),200)));
